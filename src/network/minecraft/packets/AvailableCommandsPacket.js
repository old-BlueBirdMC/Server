...
export default class AvailableCommandsPacket extends PacketsBase {
    ...
    deserialize() {
        ...
        for (let i = 0; i < this.readVarInt(); ++i) {
            this.commandData.push(this.readCommandData(this.enumValues, this.enums, this.suffixes));
        }
        for (let i = 0; i < this.readVarInt(); ++i) {
    ...
    serialize() {
        ...
        this.commandData.forEach((value) => {
            this.writeCommandData(value, this.enumValues, this.enums, this.dynamicEnums, this.suffixes);
        });
