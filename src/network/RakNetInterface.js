...
class RakNetInterface {
  ...
  constructor(server, address, rakNetMsgFH) {
    ...
    this.rakNetServer = new RakNetServer(
      address,
      ServerInfo.rakNetProtocolVersion
    );
    ...
    this.log = new Logger({
      Name: "RakNet",
      AllowDebugging: this.server.configManager.getDebug(),
      WithColors: true,
    });
  }
  ...
  async handle() {
    this.rakNetServer.on("connect", (connection) => {
      let reg = RakNetPlayerManager.registerPlayer(
        connection.address.toString(),
        new Player(connection, this.server)
      );
      if (reg !== null) {
        this.log.debug(
    ...
    this.rakNetServer.on("packet", async (stream, connection) => {
      if (stream.readUnsignedByte() === Identifiers.game) {
        const player = RakNetPlayerManager.getPlayer(
          connection.address.toString()
        );
        if (player !== null) {
          const game = new GamePacket(stream.buffer);
          await game.deserializeA();
          HandlersList.refresh(
            player,
            this.server,
            connection.address.toString()
          );
  ...
  async queuePacket(packet, player) {
    if (
      player.connection.address.toString() in
      RakNetPlayerManager.getAllWithoutEditing()
    ) {
      ...
      if (packet instanceof PacketsBase) {
        ...
        this.log.debug(
          `Failed to send an unknown packet, className: ${packet.constructor.name}`
        );
      }
    }
  }
      ...
      if (exitProcess === true) {
        ...
        for (const [, player] of RakNetPlayerManager.getAllObjectEntries()) {
          if (closeMessage === undefined) {
            ...
            player.disconnect(
              `${MinecraftTextColors.red}Server killed, reason: ${closeMessage}`
            );
          }
        }
        setInterval(async () => {
