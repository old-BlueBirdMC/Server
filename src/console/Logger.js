...
class Logger {
  ...
  debug() {
    if (this.options.AllowDebugging)
  ...
  log(msg, name, color) {
    ...
    if (this.options.WithColors === true) {
      console.log(
        `${ConsoleColors.bold}${ConsoleColors.gold}[${this.options.Name}] [${name}] ${ConsoleColors.gray}${seconds}:${minutes}:${hours} ${ConsoleColors.darkPurple}>${ConsoleColors.reset}${color}`,
        ...msg,
        `\u001b[0m`
      );
    } else {
      console.log(
        `[${this.options.Name} ${name}] ${seconds}:${minutes}:${hours} >`,
        ...msg
      );
    }
  }
